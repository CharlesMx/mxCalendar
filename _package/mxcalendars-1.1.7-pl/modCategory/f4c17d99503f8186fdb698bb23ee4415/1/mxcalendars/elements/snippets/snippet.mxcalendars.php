...
$setFeedTZ = $modx->getOption('setFeedTZ', $scriptProperties, null); //-- Example: {"5":"America/New_York"}
...
//++ Search Elements
$searchTerm = isset($_REQUEST['mxterm']) ? $_REQUEST['mxterm'] : $modx->getOption('mxterm', $scriptProperties, null);
$tplSearch = $modx->getOption('tplSearch',$scriptProperties,'search');
$searchDisplayType = isset($_REQUEST['searchDisplayType']) ? $_REQUEST['searchDisplayType'] : $modx->getOption('searchDisplayType', $scriptProperties, null);
if(!empty($searchDisplayType))
...
if(!empty($searchTerm)){
    $whereArr = array(array('title:LIKE'=>'%'.$searchTerm.'%','OR:description:LIKE'=>'%'.$searchTerm.'%','OR:location_name:LIKE'=>'%'.$searchTerm.'%','OR:location_address:LIKE'=>'%'.$searchTerm.'%' ));
}
...
$modx->setPlaceholder('mxc.searchform', $mxcal->getChunk($tplSearch,array('mxterm'=>$searchTerm,'resourceId'=>$resourceId)));
$modx->setPlaceholder('mxc.startdate', $elStartDate);
$modx->setPlaceholder('mxc.enddate', $elEndDate);
