...
function tstamptotime($ISO8601, $tstamp = null, $mergeTime = false)
{
    ...
    if (empty($ISO8601) || ($tstamp === null && $mergeTime)) {
        return false;
    }
 ...
 *
 * @param int $frequencymode
 * @param int $interval
 * @param string $frequency
 * @param null $startDate
 * @param null $endDate
 * @param array $onwd
 * @param string $occType
    ...
    */
    ...
    if (($timestamp = $startDate) === false) {
        ...
        switch ($frequencymode) {
            case 0: //Daily
                while (++$x) {
                    $occurance = mktime(date('H', $startDate), date('i', $startDate), 0, date('m', $startDate),
                        date('d', $startDate) + ($x * $interval), date('y', $startDate));
                    if ($occurance <= $endDate && $x < $frequency && $startDate < $occurance) {
                        $ar_Recur[] = $occurance;
                        if ($debug) {
                            echo $occurance . '< -is less than (jd->' . $jd . 'jdow->' . $jdow . ') -> ' . $endDate . '<br />';
                        }
                    } else {
                        if ($debug) {
                            echo $occurance . '||-is eq or greater (jd->' . $jd . '|jdow->' . $jdow . ') than -||' . $endDate . '<br />';
                        }
                        break;
                    }
                }
                ...
                while (++$x) {
                    ...
                    if (!empty($options)) {
                        ...
                        $modx->log(modX::LOG_LEVEL_ERROR,
                            '[mxHelper] _getRpeatDate: Option[' . $options['type'] . '] :: Occurance[' . $x . ']=>' . date('Y-m-d h:i a',
                                $occurance));
            ...
            case 1: //Weekly
                ...
                //-- Set new weekday values to fix mismatch in numbering of weekdays #108 #111 #116
                if (is_array($onwd)) {
                    $adjOnWd = [];
                ...
                while ($curWeek == $startWeek) {
                    ...
                    if (in_array($thisDOW, $onwd) &&
                        ...
                        if ($debug) {
                            echo $occDate . ' MATCH on $thisDOW (start week) :: CurWk=$curWeek :: StartWk=$startWeek :: NextWk=$nextWeek<br />';
                        }
            ...
            case 3: //Yearly
                while (++$x) {
                    $occurance = mktime(date('H', $startDate), date('i', $startDate), 0, date('m', $startDate),
                        date('d', $startDate), date('y', $startDate) + ($x * $interval));
                    if ($occurance <= $endDate && $x < $frequency && $startDate < $occurance) {
                        $ar_Recur[] = $occurance;
                        if ($debug) {
                            echo $occurance . '< -is less than -> ' . $endDate . '<br />';
